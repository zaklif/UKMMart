...
if ($is_ajax) {
    ...
    foreach($results as $product) {
        // Fix: Use consistent path with leading ./
        $img = (!empty($product['fld_product_image'])) ? "./images/" . $product['fld_product_image'] : "./images/default.png";
...
.search-filter-bar form {
    ...
    gap: 10px;
    align-items: center;
    width: 100%;  /* Ensures the form inside stretches full width */
}
...
<div class="container">
    ...
    ?>
        <div class="product-card">
            <img src="<?= htmlspecialchars($img) ?>" alt="<?= htmlspecialchars($product['fld_product_name']) ?>" onerror="this.src='./images/default.png'">
            <div class="card-body">
                <div>
...
function updateResults(products) {
    ...
    products.forEach(product => {
        // Fix: Don't use encodeURI on the entire path, image path is already properly formatted
        ...
        card.innerHTML = `
            <img src="${escapeHtml(imgSrc)}" alt="${escapeHtml(product.name)}" onerror="this.src='./images/default.png'">
            <div class="card-body">
                <div>
